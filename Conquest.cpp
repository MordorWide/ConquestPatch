...
static const AddressInfo GENUINE_V1_0 = {
    12566288,
...
// Function to read a sequence of bytes from memory
...
// Function to write to protected memory and restore the protection
...
const AddressInfo* determineInfoByFileSize(SIZE_T fileSize) {
    if (fileSize == GENUINE_V1_0.n_bytes) {
        return &GENUINE_V1_0;
    }
    if (fileSize == GENUINE_V1_1.n_bytes) {
        return &GENUINE_V1_1;
    }
    if (fileSize == NOCD_V1_0.n_bytes) {
        return &NOCD_V1_0;
    }
    if (fileSize == NOCD_V1_1.n_bytes) {
        return &NOCD_V1_1;
    }
...
int wmain(int argc, wchar_t* argv[]) {
    ...
    std::wstring targetProgram = L"OriginalConquest.exe";
    ...
    if ((INT_PTR)hInstance <= 32) {
        std::cerr << "Failed to launch OriginalConquest.exe. Error code: " << GetLastError() << "\n";
        return 1;
    }

    std::wcout << L"OriginalConquest.exe launched successfully." << "\n";

    // Step 3: Get the process handle to OriginalConquest.exe
    HANDLE hProcess = GetProcessHandleByName(L"OriginalConquest.exe");
    if (!hProcess) {
        std::cerr << "Could not find OriginalConquest.exe process.\n";
        return 1;
    }
